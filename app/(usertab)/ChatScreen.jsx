...
const ChatScreen = () => {
  ...
  // Initialize Firebase database using the imported app instance
    ...
    <View className={`flex-row items-end mx-3 my-2 ${item.sender === "me" ? "self-end flex-row-reverse" : "self-start"}`}>
      ...
      <View
        className={`p-3 max-w-[75%] rounded-lg flex-row items-end ${
          item.sender === "me" ? "bg-sky-950" : "bg-white border border-gray-300"
        }`}
      >
        <Text className={`${item.sender === "me" ? "text-white" : "text-gray-900"} text-lg`}>
          {item.text}
        </Text>
        {item.sender === "me" && (
          <Ionicons
