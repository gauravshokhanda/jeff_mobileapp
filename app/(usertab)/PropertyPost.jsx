...
const PropertyPost = () => {
  ...
  useEffect(() => {
    ...
    if (breakdownCostDetail && breakdownCostDetail.days) {
      setForm((prevForm) => ({
        ...prevForm,
        numberOfDays:
          breakdownCostDetail?.days?.estimated_time?.toString() || "",
  ...
  const handleSubmit = async () => {
    if (!form.numberOfDays.trim())
      return Alert.alert("Error", "Number of days is required!");
    if (!form.totalCost.trim())
      return Alert.alert("Error", "Total cost is required!");
    if (!form.zipCode.trim())
      return Alert.alert("Error", "Zip code is required!");
    ...
    if (!form.projectType.trim())
      return Alert.alert("Error", "Project type is required!");
    if (!form.description.trim())
      return Alert.alert("Error", "Description is required!");
    if (form.floorMapImages.length === 0)
      return Alert.alert("Error", "At least one floor map image is required!");
    if (form.designImages.length === 0)
      return Alert.alert("Error", "At least one design image is required!");
    ...
    try {
      const response = await axios.post(
        "https://g32.iamdeveloper.in/api/job-post",
        formData,
        {
          headers: {
      ...
      Alert.alert(
        "Success",
        "Your job application has been posted successfully!",
  ...
  const handleBackPress = () => {
    Alert.alert("Confirmation", "Are you sure you want to leave this page?", [
            ...
            <View className="p-4">
              <View className="flex-row mb-6 space-x-4">
                <View className="flex-1">
                  <Text className="pl-4 pb-2 text-gray-600">
                    Number of Days
                  </Text>
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      handleInputChange("numberOfDays", text)
                    }
                    style={{
                      elevation: 15,
                      shadowColor: "#082f49",
              ...
              <View className="flex-row mb-6 space-x-4">
                ...
                <View className="flex-1">
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      handleInputChange("projectType", text)
                    }
                  />
                </View>
              </View>
                      ...
                      <View className="border-dashed border-2 border-gray-400 p-2 rounded-lg mt-3 bg-white">
                        <View className="flex-row items-center flex-wrap mt-2">
                          {form.designImages.map((uri, index) => (
                            <View
                              key={index}
                              className="relative w-24 h-24 m-1"
                            >
                              <Image
                                source={{ uri }}
                              ...
                              <TouchableOpacity
                                onPress={() =>
                                  removeImage("designImages", index)
                                }
                          ...
                          <View className="flex-row items-center flex-wrap mt-2">
                            {form.floorMapImages.map((uri, index) => (
                              ...
                              >
                                ...
                                <TouchableOpacity
                                  onPress={() =>
                                    removeImage("floorMapImages", index)
                                  }
                                  className="absolute top-0 right-0 bg-red-500 rounded-full p-1"
                                >
                                  <Ionicons
